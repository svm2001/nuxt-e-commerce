...
<template>
    ...
    <div v-else>
        <div v-if="categoryStore.categories.length > 0" class="grid grid-cols-5 gap-5">
            ...
            >
                <Badge class="text-[14px] text-teal-300 absolute top-6 right-6 text-center">{{ category.name }}</Badge>
                <NuxtImg class="rounded-sm w-full h-full object-cover" :src="category.image" :alt="category.name" />
            </NuxtLink>
        </div>
        <div class="text-teal-300 text-3xl pt-32 text-center" v-else>
            No categories found
        </div>
    </div>
</template>