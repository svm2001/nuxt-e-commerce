...
const img = ref(props.category.image);
const close = ref<HTMLElement | null>(null)
const images = ref<File>();
const imagePreview = ref<string>(img.value);
...
const submitForm = async () => {
    ...
    try {
        ...
        const newCategory = {
            name: name.value,
            image: props.category.image !== '' ? props.category.image : imageUrl,
        };

        console.log(newCategory)

        const response = await fetch(`https://api.escuelajs.co/api/v1/categories/${props.category.id}`, {
            method: 'PUT',
            headers: {
                                ...
                                <div v-if="!imagePreview">
                                    <div class="file-input">
                                        ...
                                        <label class="file-input__label min-w-4 mr-2" for="file-input">
                                            ...
                                            <span>Upload image for category</span>
                                        </label>
                                    </div>
                                </div>
