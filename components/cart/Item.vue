    ...
    <div class="border border-teal-900 p-2.5 rounded-md flex bg-slate-900">
        <div class="w--45 flex items-center gap-2">
            ...
            <div class="flex flex-col gap-1">
                <NuxtLink :to="`/products/${props.productId}`" class="text-lg font-medium min-h-7 text-teal-400 max-h-7 overflow-hidden transition hover:text-teal-300"> {{ props.title }} </NuxtLink>
                <span class="text-slate-400 text-sm max-h-[40px] min-h-[40px] overflow-hidden"> {{ props.description }} </span>
            </div>
        </div>
        <div class="w--7 text-center flex items-center justify-center text-teal-300">
        ...
        <div class="w--16 text-center font-medium flex items-center justify-center">
            <div class="flex items-center gap-1">
                <Button class="bg-teal-300 hover:bg-teal-600 p-2" @click="cartStore.decrementCount(productId)">
                ...
                <Button class="bg-teal-300 hover:bg-teal-600 p-2" @click="cartStore.incrementCount(productId)">
                    <Icon name="mingcute:add-fill" class="w-2.5 h-2.5" style="color: black"/>
                </Button>
            </div>
        </div>
        <div class="w--7 text-center font-medium flex items-center text-teal-400 justify-center">
